...
$ok = filter_input(INPUT_POST, 'ok', FILTER_SANITIZE_SPECIAL_CHARS);
...
if ($ok) {
    try {
        ...
        $filtre = [
            'immat' => FILTER_SANITIZE_SPECIAL_CHARS,
            'modele' => FILTER_SANITIZE_SPECIAL_CHARS,
...
$modeles = $pdo->query('SELECT id_modele id, CONCAT(marque, \' \', modele, \' '
        . '(\', carburant,\')\') valeur FROM modeles;')->fetchAll(PDO::FETCH_NAMED);
...
<form action="#" method="POST">
    <table>
        ...
        <?php
        $voitures = $pdo->query('SELECT immat, id_modele, couleur, datevoiture FROM voitures;')->fetchAll(PDO::FETCH_NAMED);
