...
if ($enregistrer) {
    ...
    if ($reqM->rowCount() === 0) {
        ...
        $modele = filter_input(INPUT_POST, 'Modle', FILTER_SANITIZE_SPECIAL_CHARS);
        $carb = filter_input(INPUT_POST, 'Carb', FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        $idModele = $reqM->fetch(PDO::FETCH_NAMED)['id_modele'];
    }
    $reqM->closeCursor();
    ...
    $immat = filter_input(INPUT_POST, 'Immat', FILTER_SANITIZE_SPECIAL_CHARS);
    $date = preg_replace('#^(\d{1,2})/(\d{1,2})/(\d{4})$#', '$3-$2-$1',
            filter_input(INPUT_POST, 'Date', FILTER_SANITIZE_SPECIAL_CHARS));
